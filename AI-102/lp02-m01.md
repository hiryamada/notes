...
az cognitiveservices account create -n $name -g $name --kind TextAnalytics --sku S -l $location --yes
key=`az cognitiveservices account keys list -n $name -g $name --query 'key1' -otsv`
setx LANGUAGE_ENDPOINT "https://$location.api.cognitive.microsoft.com/"
