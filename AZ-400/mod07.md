...
(1) Skillpipe モジュール7 「アプリケーション構成とシークレットの管理 / セキュリティの概要 / SQLインジェクション攻撃」のページを参照しながら、実際にSQLインジェクション攻撃をシミュレーションしてみる。

(2) Skillpipe モジュール7 「アプリケーション構成とシークレットの管理 / 安全な開発プロセスの実装 / デモンストレーション: 脅威モデリング」のページを参照しながら、Threat Modeling Toolを使用してみる。
