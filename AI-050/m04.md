...
AIシステムに対する指示・命令。基本的には（人間の相手に作業を依頼するように）自然言語で命令すればよい。
...
なお、上記の例で `----` と書かれている部分は「セクションマーカー」（後述）。「指示」と「コンテンツ」を分けたりするのに使用される。上記では「スニペット」（プライマリコンテンツ）と、その他の命令を分けるのに使用されている。
