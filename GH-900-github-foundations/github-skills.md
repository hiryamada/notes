...
- ユーザーA: 元のリポジトリのオーナー
- ユーザーB: プルリクエスト作成者
- ユーザーBは元のリポジトリをフォークし、ブランチを作り、そこでゲームオーバーメッセージの改善を行い、コミットしている。

手順の概要:
- Step 1: ユーザーBはこの改善をプルリクエスト(PR)として送信。
- Step 2: ユーザーAはそのPRの担当者として自分自身を割り当てる
- Step 3: ユーザーAはPRにコメントする
- Step 4: ユーザーAはPR内のゲームオーバーメッセージをさらに別の表現に書き換える変更提案をする。
- Step 5: ユーザーBはその変更提案を適用する。（PRが修正される）
- Step 6: ユーザーAは、再度レビューをして、マージする。
- optional: ユーザーBは、PRがマージされたため、不要になったupdate-gameブランチを削除する。
